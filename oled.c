...
void render_layer_symbol(void) {
    ...
    if (layer_state_is(_LOWER) && !layer_state_is(_ADJUST)) {
        oled_write_P(lower_layer, false);
    } else if (layer_state_is(_RAISE) && !layer_state_is(_ADJUST)) {
...
void render_layer_name(void) {
    if (layer_state_is(_LOWER) && !layer_state_is(_ADJUST)) {
        oled_write_ln_P(PSTR("> lo "), false);
    } else if (layer_state_is(_RAISE) && !layer_state_is(_ADJUST)) {
        oled_write_ln_P(PSTR("> hi "), false);
    } else if (layer_state_is(_ADJUST)) {
        oled_write_ln_P(PSTR("> aj "), false);
    } else {
...
void render_status(void){
    ...
    render_mod_state(get_oneshot_mods());
}
